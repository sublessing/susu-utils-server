...
const susuUtils = require('susu-utils');
    ...
    query (sql) {
        try {
            ...
            this.connection.query(sql.trim(), (err, result) => {
                ...
                if (result[0] && susuUtils.Determine.isObject(result[0])) {
                    resolve(result.map((item) => {
                        let obj = {};
                        for (var name in item) {
                            obj[susuUtils.Parameter.underlineToHump(name)] = item[name];
                        }
                        return obj;
                    }));
                    return;
